/******
 *
 * 27/04/2021 - correção da instrução count para MS-SQL.
 * 03/05/2021 - incluída a verificação da instrução ORDER BY para executar o count na query.
 * 05/05/2021 - executar somente se for Admin ou grupo Admin
 * visualizar e ediar campos no formato uma coluna
 */
...
Static Function TIDevChild()
    ...
    If .NOT. FWIsAdmin(RetCodUsr())
        FwAlertWarning('Usuário não faz parte do grupo de administradores.','TiDev')
        Return
    Endif
...
Static Function PQryRun(cQuery, oPSR, np, oMsg)
    ...
    Begin Sequence
        ...
        Else
            ...
            If nSec2 >= nSec1 
                cMsgT :=  "Tempo de Execução: " + APSec2Time(nSec2 - nSec1) + " (" + Alltrim(Str(nSec2 - nSec1)) + " segs.) | Quantidade de Registros: "+Alltrim(Str(ntotRegs))
            EndIf 
            oMsg:SetText(cMsgT)
...
Static Function CountQuery(cQuery, oMsg)
	...
	If At("ORDER BY", Upper(cQuery)) > 0
		cQry := SubStr(cQuery,1,At("ORDER BY",cQuery)-1)
	Endif

    cQuery := "SELECT COUNT(*) QTDE FROM ( " + cQry + " ) " + Iif(cBco=="MSSQL","AS TMP","")
